...
const capabilities = [
  { icon: Satellite, title: 'See emissions in context.', description: 'Explore satellite observations alongside facilities and oil blocks on one map.', background: 'bg-[#eaf3ef]' },
  { icon: BarChart3, title: 'Understand what’s changing.', description: 'Compare sources, locations and time periods to investigate methane trends.', background: 'bg-[#f0f5f1]' },
  { icon: Layers3, title: 'Take the next step.', description: 'Set alerts, bring in field measurements and export evidence for your team.', background: 'bg-[#e4f0e9]' },
];
const managers = [
...
const Home: React.FC<HomeProps> = ({ isAuthenticated, onNavigate }) => {
  ...
  return (
    <main className="nogiet-public-theme nogiet-home h-screen min-h-0 overflow-x-hidden overflow-y-auto bg-[#f7faf8] text-[#142c27]">
      ...
      <header className="relative z-20 mx-auto max-w-7xl px-6 sm:px-10">
        <div className="flex h-24 items-center justify-between gap-6 border-b border-[#0b5147]/15">
          ...
          <nav aria-label="Primary navigation" className="hidden items-center gap-9 md:flex">
            {navigation.map(({ id, label }) => <a key={id} href={`#${id}`} className="text-sm text-[#37584e] hover:text-[#07574b]">{label}</a>)}
          </nav>
          ...
          <button type="button" onClick={() => setMenuOpen(!menuOpen)} aria-label={menuOpen ? 'Close navigation' : 'Open navigation'} aria-expanded={menuOpen} aria-controls="home-mobile-nav" className="rounded-full border border-[#0b5147]/25 bg-white p-3 text-[#07574b] md:hidden">{menuOpen ? <X size={20} /> : <Menu size={20} />}</button>
        </div>
        {menuOpen && (
      ...
      <section id="main-content" className="mx-auto grid max-w-[1440px] items-center gap-10 px-6 pb-16 pt-12 sm:px-10 md:py-16 lg:grid-cols-[0.85fr_1.15fr] lg:gap-12 lg:py-16">
        <div>
          <p className="inline-flex rounded-full border border-[#0b5147]/15 bg-[#e5f1e9] px-4 py-2 text-xs font-semibold uppercase tracking-[0.16em] text-[#07574b]">A clearer view. A cleaner future.</p>
          <h1 className="mt-6 max-w-2xl text-[clamp(2.75rem,5.2vw,4.6rem)] font-semibold leading-[1.07] tracking-[-0.055em]">One view of methane.<br /><span className="text-[#07574b]">One path to action.</span></h1>
        ...
        <figure className="relative isolate h-[420px] overflow-hidden rounded-[2rem] border-[6px] border-[#0b5147] bg-[#0b5147] shadow-[0_24px_65px_rgba(5,61,54,0.20)] sm:h-[540px] lg:h-[660px]">
          ...
          <figcaption className="absolute inset-x-4 bottom-4 rounded-2xl border border-[#0b5147]/10 bg-white/95 p-5 shadow-[0_12px_30px_rgba(0,0,0,0.08)] backdrop-blur-sm sm:inset-x-6 sm:bottom-6 sm:p-6">
            <div className="flex items-center gap-3"><span className="rounded-full bg-[#e5f1e9] p-2.5 text-[#07574b]"><Satellite size={21} aria-hidden="true" /></span><div><p className="text-sm font-semibold text-[#064b42]">Nigeria’s emissions, in focus.</p><p className="mt-1 text-xs leading-5 text-neutral-500">Satellite observations and field data, connected.</p></div></div>
          </figcaption>
        </figure>
      </section>

      <section id="partners" aria-labelledby="managers-title" className="border-y border-[#0b5147]/15 bg-[#edf5ef]">
      ...
      <section id="platform" className="mx-auto max-w-7xl px-6 py-20 sm:px-10 lg:py-24">
        ...
        <div className="mt-12 grid gap-5 md:grid-cols-3">
          {capabilities.map(({ icon: Icon, title, description, background }) => <article key={title} className={`rounded-3xl border border-[#0b5147]/10 p-7 sm:p-8 ${background}`}><span className="inline-flex rounded-2xl bg-white/80 p-3.5 text-[#07574b]"><Icon size={25} strokeWidth={1.5} aria-hidden="true" /></span><h3 className="mt-10 max-w-[230px] text-2xl font-semibold leading-tight tracking-[-0.03em] text-[#064b42]">{title}</h3><p className="mt-4 text-sm leading-7 text-neutral-600">{description}</p></article>)}
        </div>
      </section>
      ...
      <section id="data" className="mx-auto grid max-w-7xl gap-10 px-6 pb-20 sm:px-10 lg:grid-cols-2 lg:items-center lg:gap-24 lg:pb-24">
        <div className="rounded-3xl border border-[#0b5147]/10 bg-[#eaf3ed] p-7 sm:p-10"><p className="mb-6 text-xs font-semibold uppercase tracking-[0.14em] text-[#07574b]">Sources in one place</p><div className="grid gap-3 sm:grid-cols-2">{['Carbon Mapper', 'IMEO', 'TROPOMI', 'NASA EMIT'].map((source) => <div key={source} className="flex items-center gap-3 rounded-xl bg-white px-4 py-5 text-sm font-semibold"><Satellite size={18} strokeWidth={1.5} className="text-[#07574b]" aria-hidden="true" />{source}</div>)}</div><div className="mt-4 flex items-center gap-3 border-t border-[#0b5147]/15 pt-5 text-sm text-[#37584e]"><Layers3 size={18} aria-hidden="true" />Plus your team’s field measurements</div></div>
      ...
      <section className="mx-6 rounded-[2rem] bg-[#064b42] px-6 py-14 text-center text-white shadow-[0_20px_50px_rgba(5,61,54,0.16)] sm:mx-10 sm:py-16">
        <h2 className="text-3xl font-semibold tracking-[-0.04em] sm:text-4xl">Your next insight starts here.</h2><p className="mx-auto mt-4 max-w-md text-sm leading-7 text-white/80">Sign in to explore Nigeria’s emissions data and continue your team’s work.</p><button type="button" onClick={openPlatform} className="nogiet-button nogiet-button-primary nogiet-button-on-dark mt-7 inline-flex">{actionLabel}<ArrowRight size={18} aria-hidden="true" /></button>
      </section>
