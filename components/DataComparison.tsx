  ...
  return (
    <div className={`flex-1 overflow-y-auto p-12 transition-colors duration-300 ${darkMode ? 'bg-[#0b0e14]' : 'bg-white'}`}>
      <div className="flex justify-between items-center mb-10">
        <h1 className={`text-3xl font-extrabold tracking-tight ${darkMode ? 'text-white' : 'text-gray-900'}`}>Data Comparison</h1>
        <div className="flex gap-4">
          <button className={`px-6 py-3 border rounded-2xl font-extrabold text-sm flex items-center gap-2 shadow-sm transition-all ${darkMode ? 'bg-[#12161f] border-[#1e2430] text-gray-400 hover:bg-gray-800' : 'bg-white border-gray-100 text-gray-800 hover:bg-gray-50'
            }`}>
      ...
      <div className="flex flex-col lg:flex-row gap-10">
        ...
        <div className="w-full lg:w-[400px]">
          <div className={`rounded-[32px] shadow-sm p-10 space-y-8 border transition-colors duration-300 ${darkMode ? 'bg-[#12161f] border-[#1e2430]' : 'bg-white border-gray-50'}`}>
            <div>
            ...
            <form className="space-y-6" onSubmit={(e) => { e.preventDefault(); setHasData(true); }}>
              <div className="space-y-2">
                ...
                <select className={`w-full px-5 py-4 rounded-2xl border text-sm font-bold transition-all focus:ring-1 focus:ring-[#009688] outline-none appearance-none cursor-pointer ${darkMode ? 'bg-[#0b0e14] border-[#1e2430] text-gray-500 focus:text-white' : 'bg-[#f9faf9] border-gray-100 text-gray-400 focus:text-gray-900'
                  }`}>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <input
                    ...
                    className={`w-full px-5 py-4 rounded-2xl border text-sm font-bold transition-all focus:ring-1 focus:ring-[#009688] outline-none ${darkMode ? 'bg-[#0b0e14] border-[#1e2430] text-white placeholder-gray-700' : 'bg-[#f9faf9] border-gray-100 placeholder-gray-400 text-gray-900'
                      }`}
                  />
        ...
        <div className="flex-1">
          <div className={`rounded-[32px] shadow-sm p-12 h-full min-h-[560px] flex flex-col border transition-colors duration-300 ${darkMode ? 'bg-[#12161f] border-[#1e2430]' : 'bg-white border-gray-50'}`}>
            <h3 className={`font-extrabold text-lg mb-10 ${darkMode ? 'text-white' : 'text-gray-900'}`}>Emission Levels Comparison</h3>
            ...
            {hasData ? (
              <div className="flex-1 flex flex-col">
                ...
                <div className="flex-1 flex items-end justify-between relative pt-10 pb-10 border-b transition-colors duration-300" style={{ borderBottomColor: darkMode ? '#1e2430' : '#f9fafb' }}>
                  {/* Y-axis labels */}
                  ...
                  {months.map((month, i) => (
                    <div key={month} className="flex-1 flex flex-col items-center gap-4 group cursor-pointer">
                      <div className="flex items-end gap-1 w-full justify-center relative h-[320px]">
                        ...
                        <div className={`w-3 rounded-t-lg transition-all group-hover:opacity-80 group-hover:translate-y-[-4px] ${darkMode ? 'bg-[#009688]' : 'bg-[#002b28]'}`} style={{ height: `${(groundData[i] / 3000) * 100}%` }}></div>
                      </div>
                      <span className={`text-[12px] font-extrabold transition-colors ${month === 'Jun' ? darkMode ? 'text-[#009688]' : 'text-gray-900' : 'text-gray-300'}`}>{month}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-center space-y-8">
                <div className={`w-64 h-64 rounded-full flex items-center justify-center transition-colors duration-300 ${darkMode ? 'bg-[#0b0e14]' : 'bg-gray-50'}`}>
                  <img src="https://img.icons8.com/isometric/512/empty-box.png" alt="No data" className={`w-40 h-40 transition-all duration-300 ${darkMode ? 'opacity-20 contrast-50 invert' : 'opacity-40 contrast-75'}`} />
                </div>
                <div className="space-y-3">
                  <h4 className={`font-extrabold text-2xl tracking-tight ${darkMode ? 'text-white' : 'text-gray-900'}`}>No Data Selected</h4>
                  <p className={`text-sm font-bold max-w-xs mx-auto leading-relaxed ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>Select a facility and input ground measurements to generate a comparison report against satellite data.</p>
                </div>
              </div>
            )}
          </div>
