  ...
  return <div className="space-y-7">
    ...
    {selectedMode === "manual" && <section className={`overflow-hidden rounded-2xl border ${darkMode ? "border-[#273142] bg-[#0e141e]" : "border-slate-200 bg-white"}`}>
      ...
      <div className="grid gap-4 p-4 md:grid-cols-[minmax(0,1fr)_260px] md:p-5">
        ...
        {notice && !notice.ok && <div className="rounded-xl border border-red-500/20 bg-red-500/10 px-4 py-3 text-sm text-red-500" role="alert"><p className="font-bold">This file cannot be used</p><p className="mt-1 text-xs leading-relaxed">{notice.text}</p></div>}
    ...
    {notice && (notice.ok || selectedMode !== "manual") && <div className={`rounded-xl px-4 py-3 text-sm ${notice.ok ? "bg-emerald-500/10 text-emerald-500" : "bg-red-500/10 text-red-500"}`}>{notice.text}</div>}
