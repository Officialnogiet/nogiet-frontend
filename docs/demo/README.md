...
> **NOGIET**: *Nigerian Oil & Gas Industry Emissions Tracker*. A
...
| 03 | [03-live-map.md](./03-live-map.md) | Square emissions grid, per-source plumes, source filter, layers, search, drill-in |
...
The **?** button on each application screen opens quick tips for that screen. Select **Start guided tour** there or under **Settings → Guided tour** for a walkthrough that dims the background and highlights one section at a time. The tour runs once per signed-in account on a browser and can be restarted whenever needed. See [Dashboard Home](02-dashboard-home.md) and [Settings](11-settings.md) for practical instructions.
